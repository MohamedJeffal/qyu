...
module.exports = class Qyu extends EventEmitter {
  constructor({ rateLimit, statsInterval }) {
    ...
    this.consumerWorker = new JobConsumer(
      rateLimit,
      this.emitDoneEvent,
      this.emitErrorEvent,
      this.emitDrainEvent,
  ...
  push({ job, priority }) {
    this.producerWorker.standbyEventsEmitter.emit(
      'push',
      { job, priority },
      this.consumerWorker.executionEventsEmitter
    )
  }

  start() {
    return new Promise(resolve => {
      this.consumerWorker.executionEventsEmitter.emit('start', resolve)
      this.consumerWorker.executionEventsEmitter.emit(
        'pull',
        this.producerWorker.standbyEventsEmitter
      )
    })
  }
