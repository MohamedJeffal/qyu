...
module.exports = class JobConsumerWorker {
   ...
   * @returns {boolean} Return whether the consumer will try to pull new jobs
   */
  ...
  /**
   * After jobs are pulled (either through events or the internal buffer), this will be called with the pulled data, and:
   ...
   * @returns {void}
   */
  onPulled(pullEvent, { jobs = [], remainingJobCount = 0 } = {}) {
    if (
      jobs.length === 0 &&
      remainingJobCount === 0 &&
      this.runningJobsCount === 0
   ...
   * @param {function} pullEvent - A function that emits an event to the consumer facade, used for each executed job to pull new jobs
   * @returns {Promise<Array<void>>} Return a promise resolved when all jobs are executed
   */
  executeJobs(jobs, pullEvent) {
    this.runningJobsCount += jobs.length

    return Promise.all(
      jobs.map(job => this.executeJob(job).then(() => pullEvent()))
    )
  }
